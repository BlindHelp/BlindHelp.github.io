...
-   [8. Déplacement du curseur](#HDR-MovingCursor)
    ...
    -   [8.3. Déplacement du curseur à l\'aide de la boite de sélection
        ...
        -   [8.3.1. Raison pour l\'utilisation de l\'option
            \"durée\"](#toc40)
    -   [8.4. Longueur des sauts faits avec les flèches droite et
        gauche](#HDR-JumpSize)
...
-   [la liste de diffusion anglophone
    ...
    l\'une ou l\'autre méthode, vous recevrez un e-mail pour confirmer
...
[]{#HDR-MovingCursorWithSelectionStart}
...
-   Définir la fin de la sélection à l\'aide du raccourci \"\]\" (voir
    ...
    -   Appuyez sur \"Espace\" pour positionner le curseur de lecture au
        début de la sélection si vous voulez écouter l\'audio
...
[]{#HDR-UsingStoredPositionTimeRangeSelection}
...
-   suivant que vous voulez les déplacer dans le temps, vers l\'avant ou
    ...
    -   tapez \"flèche gauche\" pour se positionner sur le chiffre des
        dixièmes.
    -   tapez \"flèche haut\" cinq fois pour incrémenter ce chiffre de
        cinq unités.
...
-   Pause maximale. Si le temps entre deux périodes où le niveau de la
    ...
    vers le bas. Si, par exemple, la piste de contrôle contient de la
    parole, alors s\'il y a des pauses inférieures à la pause maximale,
    alors l\'audio sélectionné continue d\'être atténué pendant cette
    période. La valeur par défaut de la pause maximale est de 1 seconde.
    Notez que quel que soit ce réglage, la pause maximale est toujours
    au moins égale à la somme des longueurs de fondu externes, qui sont
    décrites ci-dessous.
-   Longueurs de fondu. Le temps pendant lequel le volume de l\'audio
    ...
    Le volume de l\'audio sélectionné peut commencer à baisser avant que
    le niveau de la piste de contrôle ne commence à dépasser le seuil.
    Cet intervalle de temps est connu sous le nom de longueur de fondu
    extérieur, car il se produit en dehors du temps où le seuil est
    dépassé et sa valeur par défaut est de 0,5 seconde. De plus, le
    volume peut finir de baisser après que le seuil commence à être
    dépassé. Cet intervalle est connu sous le nom de longueur de fondu
    intérieur, car il se produit à l\'intérieur du temps où le seuil est
    dépassé, et sa valeur par défaut est de 0 seconde. De manière tout à
    fait similaire, le temps pendant lequel le volume de l\'audio
    sélectionné est ramené en arrière est également composé de deux
    parties : les longueurs de fondu vers le haut interne et externe,
    qui ont des valeurs par défaut de 0 et 0,5 seconde respectivement.
...
-   Ajouter un marqueur à la Sélection (raccourci \"control + B\"): sans
    ...
    du Curseur est créé. S\'il y a une plage de temps sélectionnée, un
    marqueur de région pour cet intervalle de temps est créé.
...
-   Tapez le nom du marqueur (facultatif), puis appuyez sur \"Entrée\"
    ...
    la boîte de dialogue. Le nouveau marqueur est ajoutée à une piste de
    marqueurs, ce qui est :
    -   soit la piste focalisée, s\'il s\'agit d\'une piste de
        marqueurs.
    -   soit la première piste de marqueurs en dessous de la piste
        focalisée, si une telle piste existe.
    -   soit une nouvelle piste de marqueurs ajoutée aux pistes
        existantes.
...
-   Il y a alors deux solutions:
    ...
    -   La deuxième est d\'utiliser la touche d\'accès rapide \"alt+s\"
...
-   Assurez-vous que toutes les pistes à l\'exception de la piste de
    marqueur contenant le marqueur ne sont pas sélectionnées. Peu
    importe que la piste de marqueur elle-même soit sélectionnée ou non.
-   Sélectionnez un marqeur à l\'aide des touches Tab et Maj + Tab,
...
-   Dans le menu Edition, choisissez Renommer le clip (Ctrl + F2). Une
    boîte de dialogue Set Wave Clip Name s\'ouvre et le contenu de la
    zone d\'édition du nom est initialement sélectionné.
-   Tapez un nom et appuyez sur Entrée.

[]{#toc101}

### 13.3. Diverses commandes de découpe

[]{#toc102}

#### 13.3.1. Fractionnement
...
#### 13.3.2. Commandes \"Supprimer-séparer\" et \"couper-séparer\"
...
#### 13.3.3. Scinder dans une nouvelle piste
...
Une nouvelle piste est ajouté après la dernière piste, et l\'audio
...
[]{#HDR-ClipTimeShifting}

### 13.5. Décalage temporel des clips
...
#### 13.5.1. Déplacement d\'un seul clip
...
fin de un clip et le début du clip suivant, pour les commandes de
...
#### 13.5.2. Déplacement de plusieurs clips adjacents
...
#### 13.5.3. Déplacement de clips avec le verrouillage de Synchronisation des pistes activée
...
Si le verrouillage de synchronisation des pistes est activée, lorsque
...
-   Donc, pour ouvrir la boîte de dialogue pour l\'effet ajouté, appuyez
    ...
    ++ Boîte de dialogue d\'effet en temps réel ++ Le titre d\'une boîte
    de dialogue d\'effet en temps réel se compose du nom de l\'effet,
    suivi d\'un tiret, suivi du nom de la piste. Et la boîte de dialogue
...
-   La seconde consiste à utiliser les deux touches standard mais peu
    ...
    respectivement. Sur les claviers britanniques et américains, vous
    pouvez utiliser les deux touches à droite de la touche zéro sur la
    rangée supérieure du clavier pour ces touches.
...
## 15. Édition des Balises de métadonnées
...
### 15.2. Modification des valeurs de paramètre
...
### 15.3. Création et modification de balise
...
[]{#HDR-EditingGenreList}

### 15.4. Édition de la liste des genres
...
-   la boîte de dialogue d\'édition des genres s\'ouvre. Le premier
    élément de la boite est une zone d\'édition multi-ligne avec un
    genre par ligne. Notez que lors de la première ouverture de la boîte
    de dialogue, tous les genres sont sélectionnés, et donc, sauf si
    vous tapez d\'abord une touche de navigation, par exemple \"Flèche
    droite\", vous remplacerez tous les genres. Pour vous déplacer vers
    le haut ou le bas de la liste, appuyez sur \"control + début\" ou
...
### 16.1. Paramètres d\'enregistrement
...
#### 16.1.2. Périphérique d\'enregistrement
...
#### 16.1.3. Niveau d\'enregistrement
...
#### 16.1.4. Nombre de canaux d\'enregistrement
...
#### 16.1.5. Option Doublage (Overdub)
...
#### 16.1.6. Option Lecture à travers le logiciel
...
### 16.2. Commandes d\'enregistrement
...
### 16.3. Réglage du niveau d\'enregistrement
...
vu-mètre contient un bouton déroulant que vous pouvez atteindre en
...
### 16.4. Enregistrement Punch and Roll
...
#### 16.5.1. Générer une piste de rythme
...
-   la piste de rythme est créée. La piste est initialement sélectionné,
    et une plage de temps est choisie, qui comprend toutes les données
    audio.

[]{#toc131}

#### 16.5.2. Enregistrer la piste de rythmes
...
-   Avec la première piste toujours sous le focus, appuyez sur
    ...
    -   Dans la boite de dialogue \"Amplifier\", appuyez sur \"Entrée\"
        pour accepter la valeur par défaut.

[]{#toc132}

#### 16.5.3. Trouver la position d\'un clic sur la piste enregistrée
...
-   Premier cas: trouver la position du clic enregistré quand il arrive
    ...
    -   recommencer l\'étape précédente jusqu\'à ce que le temps qui
        s\'écoule jusqu\'au premier clic, passe brusquement à environ
        une seconde. Vous avez dépassé le premier clic.
...
### 17.2. Plugins d\'effets VST et VST3
...
-   Pour que le changement d\'interface prenne effet, vous devez alors
    ...
    ++ Dialogue d\'interface dans l\'effet ++ Presque tous les effets
    ...
    ci-dessous n\'a aucun effet.
    Heureusement, les plug-ins VST3 ne sont pas affectés et la plupart
    des plug-ins VST sont désormais disponibles en tant que plug-ins
    VST3.
    Lorsque vous ouvrez un effet VST ou VST3 pour la première fois, son
    interface utilisateur graphique personnalisée est utilisée et vous
    ne pourrez accéder qu\'à un petit nombre de commandes d\'effets,
    telles que le bouton Préréglages et paramètres. Pour passer à
    l\'utilisation d\'une interface utilisateur accessible par clavier :
-   Appuyez sur le bouton Préréglages et paramètres et choisissez
    \"Options\". Une boîte de dialogue d\'options d\'effet s\'ouvre.
-   Tabulez jusqu\'à la case à cocher \"activer l\'interface
    graphique\", puis décochez-la.
...
### 18.1. Boîte de dialogue Préférences
...
[]{#HDR-MetaDataTagEditor}

### 18.6. Ouverture automatique de l\'éditeur de Balises de métadonnées à l\'exportation audio
...
### 18.8. Option \"Utiliser un dialogue pour nommer un nouveau marqueur\"
...
### 18.9. Taper pour créer un marqueur
...
[]{#HDR-PastingAudioFromOtherProject}

### 18.10. Coller l\'audio d\'un projet différent
...
### 18.11. Déplacement des clips suivants lors de l\'ajout ou suppression de l\'audio
...
### 18.12. Tri ou regroupement du menu Effet
...
### 18.13. Options d\'enregistrement
...
### 18.14. Modification ou création de raccourcis clavier
...
-   Une arborescence ou une liste de commandes, suivant la vue défini
    par le bouton radio.

[]{#toc151}

#### 18.14.1. Vue en arborescence
...
#### 18.14.5. Création ou modification d\'un raccourci
...
-   tabulez jusqu\'au bouton \"Attribuer\" puis appuyez dessus. Si le
    raccourci est déjà affecté à une autre commande, une boîte de
    dialogue d\'Erreur s\'ouvre qui vous dit le nom de cette commande.
...
[]{#HDR-DefaultShortcutsSet}

### 18.15. Jeu de raccourcis standard ou complet
...
[]{#HDR-BracketShortcuts}

### 18.16. Les Raccourci gênants avec un clavier Azerty

[]{#toc158}

#### 18.16.1. les crochets gauche et droite
...
Pour les clavier de type Azerty,il est donc préférable de changer ces
raccourcis.
...
[]{#HDR-UpdateAutomaticChecking}

### 18.17. Vérification automatique des mises à jour
...
[]{#HDR-SoundWindowsDialog}

## 19. Boite de dialogue Son de Windows
...
### 19.3. Boite de dialogue Propriétés d\'un périphérique
...
### 20.2. Commandes relatives à la lecture audio
...
### 20.3. Commandes relatives au Tableau de pistes
...
### 20.4. Commandes relatives à une piste Audio
...
### 20.5. Commandes relatives au Déplacement du curseur
...
### 20.6. Commande relatives à la sélection d\'une plage de temps
...
### 20.7. Commandes relatives à l\'édition audio
...
### 20.8. Commandes relatives aux Marqueurs
...
### 20.9. Commandes relatives aux Clips
...
### 20.10. Commandes relatives à l\'enregistrement audio
