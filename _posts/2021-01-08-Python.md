---
title: Installation de Python, de pip et de l'interface de ligne de commande SCons et Markdown sous Windows
permalink: "/Python/"
layout: post
author: BlindHelp
---
...
Voici un nouveau tuto plus bas fait par mes soins, Cela traitera la façon de mettre à jour les dépendances utilisées pour la construction d'une  extensionpour NVDA,   cela est complémentaire  aux   quatre  tuto que j'ai posté il y a longtemps, ceux-ci déclinable en quatre parties consultable via les liens suivants:       
...
J'ai remarqué quand je voulais construire une extension d'un ami brésilien traduit  à l'Espagnol, lors de l'invite de commande quand j'ai tapé:    
`¤ Invite de commande`    
`scons`    
ou
`¤ Invite de commande`    
`scons pot`    
 ont été affichés des messages d'avertissement  et j'étais dans l'impossibilité de créer l'extension ou créer le fichier de message.    
Ce même problème n'a pas aparu quand j'ai fait la même chose avec des extensions précédentes des autres amis développeurs d'extensions.    
En regardant bien le dépôt GitHub de cet ami brésilien, j'ai remarqué le fichier appelé "contributing" et j'ai vu que les modules déjà énumérés ci-dessus doivent être configurés.
Donc, sans trop tardé, j'ai installé et j'ai mis à jour les modules, et lorsque j'ai fait les mêmes commandes ci-dessus, j'ai pu construire ladite extension et créer le fichier .pot, c'est-à-dire le catalogue de messages à utiliser avec POEDIT.    
Maintenant, je partage cela avec mes amis  développeurs et traducteurs d'extension pour NVDA.    
...
Si vous utilisez Python 3.2, vous aurez besoin de cette version de get-pip.py au lieu. Dans tous les cas, cliquez avec le bouton droit sur le lien et sélectionnez Enregistrer sous...  et enregistrez-le dans un emplacement sûr, tel que votre dossier Téléchargements.
...
N'oubliez pas de mettre à jour vos paquets PIP, SCons et Markdown de temps en temps en utilisant pip.    
`¤ Invite de commande`    
`python -m pip install --upgrade pip scons markdown`    
...
# Nouveau fichier sconstruct révisé et corrigé  par Paulber19 #
