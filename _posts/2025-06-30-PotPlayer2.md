...
ou collez l'chemin de ce fichier dans la zone d'édition appelé:    
...
`Maj+E Utiliser l'égaliseur`    

Appuyez sur ce raccourci clavier pour activer ou désactiver l'égaliseur natif de PotPlayer.    

## Comment accéder à l'égaliseur à partir des `Préférences… F5` (raccourci par défaut).
...
## Comment naviguer à travers des préréglages de l'égaliseur choisi (ceci depuis `Préférences… F5` (raccourci par défaut).    

De la même manière que vous avez navigué à travers de la liste déroulante pour alterner entre les différents égaliseurs disponibles, Vous pouvez également naviguer à travers des préréglages de l'égaliseur choisi (ceci depuis `Préférences… F5` (raccourci par défaut).    
...
Si vous êtes sûr et que vous êtes conforme avec ces modifications, vous pouvez appuyer sur le bouton "OK", mais si vous avez un doute sur ces modifications, pour annuler ces modifications, appuyez sur le bouton "Annuler".    
